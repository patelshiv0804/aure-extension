    ...
    if (response?.data) {
      const mappedPrompts = response.data.map((p) => {
        ...
        return {
          ...
          category,
          mode,
          platform: (p.ai_model?.provider && p.ai_model.provider.toLowerCase() !== 'promptiq') ? p.ai_model.provider : '',
          aiModel: (p.ai_model?.model_name && p.ai_model.model_name.toLowerCase() !== 'promptiq') ? p.ai_model.model_name : '',
